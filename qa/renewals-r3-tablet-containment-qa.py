...
try:
    ...
    metrics = driver.execute_script(
        ...
        const wrap = document.querySelector('.r3-renewals-directory .r3-case-table-wrap');
        const table = document.querySelector('.r3-renewals-directory .r3-case-table');
        const head = document.querySelector('.r3-renewals-directory .r3-case-table thead');
    ...
    if page_overflow > TOLERANCE_PX:
        raise AssertionError(f"Renewals directory page overflow at 1024px: {page_overflow}px")
    if table_overflow > TOLERANCE_PX:
        raise AssertionError(f"Renewals directory internal table overflow at 1024px: {table_overflow}px")
    if metrics["theadDisplay"] != "none":
        raise AssertionError(f"1024px Renewals directory must hide table header: {metrics['theadDisplay']}")
    if metrics["rowDisplay"] != "grid":
        raise AssertionError(f"1024px Renewals directory must render cards: {metrics['rowDisplay']}")
    if metrics["tableMinWidth"] != "0px":
        raise AssertionError(f"1024px Renewals directory must remove table min-width: {metrics['tableMinWidth']}")

    print(
        json.dumps(
            {
                "event": "RENEWALS_R3_TABLET_CONTAINMENT_PASS",
                **metrics,
