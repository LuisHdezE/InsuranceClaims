...
try:
    ...
    metrics = driver.execute_script(
        ...
        const wrap = document.querySelector('.r3-collections-directory .r3-case-table-wrap');
    ...
    if page_overflow > TOLERANCE_PX:
        raise AssertionError(f"Collections directory page overflow at 1024px: {page_overflow}px")
    if table_overflow > TOLERANCE_PX:
        raise AssertionError(f"Collections directory internal table overflow at 1024px: {table_overflow}px")
    if metrics["theadDisplay"] != "none":
        raise AssertionError(f"1024px Collections directory must hide table header: {metrics['theadDisplay']}")
    ...
    print(
        json.dumps(
            {
                "event": "COLLECTIONS_R3_TABLET_CONTAINMENT_PASS",
                **metrics,
