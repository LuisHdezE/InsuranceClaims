...
try:
    driver.execute_cdp_cmd("Emulation.setDeviceMetricsOverride", {"width": 1024, "height": 768, "deviceScaleFactor": 1, "mobile": False})
    ...
    metrics = driver.execute_script(
        ...
        const wrap = document.querySelector('.r3-policy-directory .cp360-table-wrap');
        const table = document.querySelector('.r3-policy-directory .cp360-table');
        const head = document.querySelector('.r3-policy-directory .cp360-table thead');
    ...
    if page_overflow > TOLERANCE_PX:
        raise AssertionError(f"Policy directory page overflow at 1024px: {page_overflow}px")
    if table_overflow > TOLERANCE_PX:
        raise AssertionError(f"Policy directory internal table overflow at 1024px: {table_overflow}px")
    if metrics["theadDisplay"] != "none":
        raise AssertionError(f"1024px policy directory must hide table header: {metrics['theadDisplay']}")
    if metrics["rowDisplay"] != "grid":
        raise AssertionError(f"1024px policy directory must render cards: {metrics['rowDisplay']}")
    if metrics["tableMinWidth"] != "0px":
        raise AssertionError(f"1024px policy directory must remove table min-width: {metrics['tableMinWidth']}")

    print(json.dumps({"event": "POLICIES_R3_TABLET_CONTAINMENT_PASS", **metrics, "pageOverflow": page_overflow, "tableOverflow": table_overflow}, ensure_ascii=False))
finally:
    try:
