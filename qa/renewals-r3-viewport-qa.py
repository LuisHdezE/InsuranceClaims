...
RENEWAL_ID = "renewal-visual-001"
TARGETS = ((1366, 768), (1280, 720), (1024, 768), (390, 844))
TOLERANCE_PX = 2
ARTIFACT_DIR = Path(".qa-artifacts/renewals-r3")
...
try:
    ...
    renewals_link = wait.until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, "a.operator-sidebar-item[href='/operator/renewals']"))
    )
    driver.execute_script("arguments[0].scrollIntoView({block:'center'});", renewals_link)
    renewals_link.click()
    wait.until(lambda d: d.current_url.endswith("/operator/renewals"))
    wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, ".r3-renewals-directory")))
    wait.until(EC.visibility_of_element_located((By.XPATH, "//h1[normalize-space()='Renovaciones']")))
    ...
    wait.until(lambda d: "En cotización" in d.find_element(By.CSS_SELECTOR, ".r3-case-table").text)
    ...
    for width, height in TARGETS:
        ...
        metrics = driver.execute_script(
            ...
            const page = document.querySelector('.r3-renewals-directory');
            const title = document.querySelector('.r3-renewals-directory .r3-case-page-heading h1');
            const summary = document.querySelector('.r3-renewals-directory .r3-case-heading-summary');
            const refresh = document.querySelector('.r3-renewals-directory .ops-refresh-button');
            const tableWrap = document.querySelector('.r3-renewals-directory .r3-case-table-wrap');
            const table = document.querySelector('.r3-renewals-directory .r3-case-table');
            const thead = document.querySelector('.r3-renewals-directory .r3-case-table thead');
            const row = document.querySelector('.r3-renewals-directory .r3-case-table tbody tr');
            const action = document.querySelector('.r3-renewals-directory .r3-case-row-action');
            return {
        ...
        shot, full = capture(f"renewals-directory-{width}x{height}")

        if overflow > TOLERANCE_PX:
            raise AssertionError(f"Renewals directory horizontal overflow at {width}x{height}: {overflow}px")
        if width >= 1200:
            if metrics["titleFont"] > 29:
                raise AssertionError(f"Renewals directory title is oversized: {metrics['titleFont']}px")
            if metrics["rowHeight"] > 72:
                raise AssertionError(f"Renewals directory row lost approved density: {metrics['rowHeight']}px")
            if metrics["tableDisplay"] != "table":
                raise AssertionError(f"Desktop Renewals directory must remain a table: {metrics['tableDisplay']}")
        if width <= 1100:
            if metrics["theadDisplay"] != "none":
                raise AssertionError("Narrow Renewals directory must hide the table header and render cards")
            if metrics["rowDisplay"] != "grid":
                raise AssertionError(f"Narrow renewal rows must render as cards: {metrics['rowDisplay']}")
            if metrics["tableScrollWidth"] - metrics["tableClientWidth"] > TOLERANCE_PX:
                raise AssertionError("Narrow Renewals directory must not rely on internal horizontal scrolling")
        if width <= 620:
            for name, value in (("refresh", metrics["refreshHeight"]), ("case action", metrics["actionHeight"])):
                if value < 44:
                    raise AssertionError(f"Renewals mobile {name} target is too short: {value}px")
            if metrics["titleFont"] > 23:
                raise AssertionError(f"Renewals mobile title is oversized: {metrics['titleFont']}px")
    ...
    detail_link = wait.until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, f"a[href='/operator/renewals/{RENEWAL_ID}']"))
    )
    ...
    for width, height in TARGETS:
        ...
        metrics = driver.execute_script(
            ...
            const root = document.documentElement;
            const body = document.body;
            const page = document.querySelector('.r3-renewal-detail');
            const title = document.querySelector('.r3-renewal-detail .r3-case-hero h1');
            const hero = document.querySelector('.r3-renewal-detail .r3-case-hero');
            const detailGrid = document.querySelector('.r3-renewal-detail .r3-case-detail-grid');
            const pipelineGrid = document.querySelector('.r3-renewal-detail .r3-case-pipeline-layout');
            const customerAction = document.querySelector('.r3-renewal-detail .r3-case-context-grid > div:first-child a');
            const policyAction = document.querySelector('.r3-renewal-detail .r3-case-context-grid > div:nth-child(2) a');
            const lifecycleAction = document.querySelector('.r3-renewal-detail .r3-case-decision');
            const stageAction = document.querySelector('.r3-renewal-detail .r3-case-stage-actions button');
            return {
              innerWidth: window.innerWidth,
              innerHeight: window.innerHeight,
              scrollWidth: Math.max(root.scrollWidth, body.scrollWidth),
        ...
        overflow = metrics["scrollWidth"] - metrics["innerWidth"]
        shot, full = capture(f"renewal-detail-{width}x{height}")

        if overflow > TOLERANCE_PX:
            raise AssertionError(f"Renewal detail horizontal overflow at {width}x{height}: {overflow}px")
        detail_columns = track_count(str(metrics["detailGridColumns"]))
        pipeline_columns = track_count(str(metrics["pipelineGridColumns"]))
        if width >= 1200:
            if metrics["titleFont"] > 29:
                raise AssertionError(f"Renewal detail title is oversized: {metrics['titleFont']}px")
            if metrics["heroHeight"] > 175:
                raise AssertionError(f"Renewal detail hero wastes vertical space: {metrics['heroHeight']}px")
            if detail_columns < 2:
                raise AssertionError(f"Desktop renewal detail lost two-column context/lifecycle composition: {metrics['detailGridColumns']}")
            if pipeline_columns < 2:
                raise AssertionError(f"Desktop renewal pipeline lost two-column composition: {metrics['pipelineGridColumns']}")
        elif width <= 1100:
            if detail_columns != 1:
                raise AssertionError(f"Narrow renewal detail must stack context/lifecycle panels: {metrics['detailGridColumns']}")
            if pipeline_columns != 1:
                raise AssertionError(f"Narrow renewal pipeline must stack: {metrics['pipelineGridColumns']}")
        if width <= 620:
            for name, value in (
                ...
                if value and value < 44:
                    raise AssertionError(f"Renewal detail mobile {name} target is too short: {value}px")
            if metrics["titleFont"] > 23:
                raise AssertionError(f"Renewal detail mobile title is oversized: {metrics['titleFont']}px")
...
print(
    json.dumps(
        {
            "event": "RENEWALS_R3_VIEWPORT_PASS",
