...
export function AdminCustomFieldDetailPage() {
  ...
  return (
    <OperatorShell>
      <main className="operator-main ops-main cf-admin-main custom-fields-r3-detail">
        <div className="cf-breadcrumbs"><Link to="/operator/admin/custom-fields">Campos personalizados</Link><span>/</span><span>{field?.fieldKey ?? definitionId.slice(0, 8)}</span></div>
        ...
        {fieldQuery.isLoading ? (
          <div className="ops-compact-empty" role="status">Cargando campo personalizado…</div>
        ) : !field ? null : (
          <>
            <section className="cf-admin-hero" aria-labelledby="cf-admin-title">
              <div>
                <span className="ops-kicker">Definición de campo personalizado R3</span>
                ...
                <div className="cf-admin-hero-meta">
                  ...
                  <details className="cf-technical-details">
            ...
            <section className="ops-panel cf-version-panel" aria-labelledby="cf-versions-title">
              <div className="ops-panel-heading">
                <div>
                  ...
                  <p>fieldKey y target forman la identidad estable. Todo cambio de contenido crea una nueva versión DRAFT inmutable.</p>
                </div>
...
function CustomFieldVersionCard({
  ...
  return (
    <article className={`cf-version-card${active ? ' is-active' : ''}`}>
      ...
      <div className="cf-token-summary">
        <span>Metadata de validación · validationMetadata ({metadata.length})</span>
        {metadata.length > 0
