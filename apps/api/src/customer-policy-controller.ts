...
export class CustomerPolicyController {
  ...
  async listCustomers(@Query() query: Record<string, string | undefined>, @Req() req: any) {
    ...
    if (isDemoModeEnabled() && isPublicDemoOperator(req.actor)) {
      ...
      const source = await this.runtime.customerPolicy.listCustomers({ ...parsed, page: 1, pageSize: 100 }, req.actor);
      return scopePublicDemoPage(source.items, 'customer', (item) => item.customerId, page, pageSize);
    }
  ...
  async listPolicies(@Query() query: Record<string, string | undefined>, @Req() req: any) {
    ...
    if (isDemoModeEnabled() && isPublicDemoOperator(req.actor)) {
      const page = parsed.page ?? 1;
      const pageSize = parsed.pageSize ?? 25;
      const source = await this.runtime.customerPolicy.listPolicies({ ...parsed, page: 1, pageSize: 100 }, req.actor);
      return scopePublicDemoPage(source.items, 'policy', (item) => item.policyId, page, pageSize);
    }
